...
#Remove any specials symbols from songs name
...
#For Pausing Song
def Pause():
         #variable to store current time of song paused
         global pausedTime
         global pause,pauseButtonText
         #if the song is paused it will toggle to resume
         if pause:
                  Resume()
                  return
         #Pause the song
         ...
         pause=True
         #whenever paused the pause button text is changed to resume
         PauseBtn["text"]="Resume"

#For resuming song
def Resume():
         #variables to store current time when resume
...
#whenever the next song is to be played this operation will happen which will move songs on particular folder based on the condition
...
#this will keep track of time like for how much time the song is played
...
#if we like the song but don't wanna listen right now the this function will be called which helps to move song in listening folder
def MoveToListening():
         global skiptime
         skiptime+=60
         Next()

#To extract all files from the given location
def GetAllFiles():
         global songList
         for root,dirs,files in os.walk(os.getcwd(),topdown=True):
                  songList=files
...
#whenever the song is finished next song is played automatically
def PlayNextSongAutomatically():
         global songended
         #will continiously check whether the song is playing or not
         while True:      
                  try:
                    for e in pygame.event.get():
                      if e.type==SONG_END:
                           songended=True
                           if totalSongs>1:
                                    #if song is finished, call the action function to move the current song to listened folder
...
def Play():
         ...
         try:
                  ...
                  #will continiously check whether the current song ended or not
                  threading.Thread(target=PlayNextSongAutomatically).start()
                  
                  PlayTimeEnd=1
                  #pause the program for a second to allow operation to finish in timecounter function
                  time.sleep(1);
                  if totalSongs>1:
...
#whenever the first song is played this function will be called which will create the folders for song grouping as well
def PlayFirst():
         global totalSongs
         SecondWindow()
